...
const family = findFamily('sharp');

if (!family) {
  console.error('iconify-json-nucleo-sharp: unknown family "sharp"');
  process.exit(1);
}
...
try {
  ...
  console.log(`iconify-json-nucleo-sharp: built ${iconCount} icons for "${family.prefix}"`);
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`iconify-json-nucleo-sharp: ${message}`);
