# iconify-json-nucleo-micro-bold

> Nucleo **micro-bold** icons as an [Iconify](https://iconify.design/) icon set.
...
npm install iconify-json-nucleo-micro-bold
```
...
| ---------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| npm                                | Yes, until npm v12 | From v12 (expected July 2026), run `npm approve-scripts iconify-json-nucleo-micro-bold`                                               |
| pnpm 11                            | No                 | Add `iconify-json-nucleo-micro-bold: true` under `allowBuilds` in `pnpm-workspace.yaml`, or run `pnpm approve-builds`                 |
| pnpm 10                            | No                 | Add `iconify-json-nucleo-micro-bold` to `pnpm.onlyBuiltDependencies` in `package.json`, or run `pnpm approve-builds`                  |
| Yarn 4.14 and newer                | No                 | Add `dependenciesMeta.iconify-json-nucleo-micro-bold.built: true` to `package.json`                                                   |
| Yarn Classic and Berry before 4.14 | Yes                | Nothing needed                                                                                                                        |
| Bun                                | No                 | Add `iconify-json-nucleo-micro-bold` to `trustedDependencies` in `package.json`, or run `bun pm trust iconify-json-nucleo-micro-bold` |

To skip the postinstall, generate the set on demand:

```bash
npx iconify-json-nucleo-codegen build --base node_modules/iconify-json-nucleo-micro-bold
```
...
```ts
// ESM
import { icons } from 'iconify-json-nucleo-micro-bold';
...
const icons = require('iconify-json-nucleo-micro-bold');
...
```tsx
<Icon icon="nucleo-micro-bold:heart-20" />
```

With [`@iconify/tailwind`](https://iconify.design/docs/usage/css/tailwind/) the JSON is read straight from `node_modules`.

```html
<span class="icon-[nucleo-micro-bold--heart-20]"></span>
```

Build tools that need the raw set can read it directly from the `iconify-json-nucleo-micro-bold/icons.json` subpath export.
