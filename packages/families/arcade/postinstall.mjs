...
const family = findFamily('arcade');

if (!family) {
  console.error('iconify-json-nucleo-arcade: unknown family "arcade"');
  process.exit(1);
}
...
try {
  ...
  console.log(`iconify-json-nucleo-arcade: built ${iconCount} icons for "${family.prefix}"`);
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`iconify-json-nucleo-arcade: ${message}`);
