# iconify-json-nucleo-glass

> Nucleo **glass** icons as an [Iconify](https://iconify.design/) icon set.
...
npm install iconify-json-nucleo-glass
```
...
| ---------------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| npm                                | Yes, until npm v12 | From v12 (expected July 2026), run `npm approve-scripts iconify-json-nucleo-glass`                                          |
| pnpm 11                            | No                 | Add `iconify-json-nucleo-glass: true` under `allowBuilds` in `pnpm-workspace.yaml`, or run `pnpm approve-builds`            |
| pnpm 10                            | No                 | Add `iconify-json-nucleo-glass` to `pnpm.onlyBuiltDependencies` in `package.json`, or run `pnpm approve-builds`             |
| Yarn 4.14 and newer                | No                 | Add `dependenciesMeta.iconify-json-nucleo-glass.built: true` to `package.json`                                              |
| Yarn Classic and Berry before 4.14 | Yes                | Nothing needed                                                                                                              |
| Bun                                | No                 | Add `iconify-json-nucleo-glass` to `trustedDependencies` in `package.json`, or run `bun pm trust iconify-json-nucleo-glass` |

To skip the postinstall, generate the set on demand:

```bash
npx iconify-json-nucleo-codegen build --base node_modules/iconify-json-nucleo-glass
```
...
```ts
// ESM
import { icons } from 'iconify-json-nucleo-glass';
...
const icons = require('iconify-json-nucleo-glass');
...
```tsx
<Icon icon="nucleo-glass:heart-24" />
```

With [`@iconify/tailwind`](https://iconify.design/docs/usage/css/tailwind/) the JSON is read straight from `node_modules`.

```html
<span class="icon-[nucleo-glass--heart-24]"></span>
```

Build tools that need the raw set can read it directly from the `iconify-json-nucleo-glass/icons.json` subpath export.
