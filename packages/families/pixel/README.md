# iconify-json-nucleo-pixel

> Nucleo **pixel** icons as an [Iconify](https://iconify.design/) icon set.
...
npm install iconify-json-nucleo-pixel
```
...
| ---------------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| npm                                | Yes, until npm v12 | From v12 (expected July 2026), run `npm approve-scripts iconify-json-nucleo-pixel`                                          |
| pnpm 11                            | No                 | Add `iconify-json-nucleo-pixel: true` under `allowBuilds` in `pnpm-workspace.yaml`, or run `pnpm approve-builds`            |
| pnpm 10                            | No                 | Add `iconify-json-nucleo-pixel` to `pnpm.onlyBuiltDependencies` in `package.json`, or run `pnpm approve-builds`             |
| Yarn 4.14 and newer                | No                 | Add `dependenciesMeta.iconify-json-nucleo-pixel.built: true` to `package.json`                                              |
| Yarn Classic and Berry before 4.14 | Yes                | Nothing needed                                                                                                              |
| Bun                                | No                 | Add `iconify-json-nucleo-pixel` to `trustedDependencies` in `package.json`, or run `bun pm trust iconify-json-nucleo-pixel` |

To skip the postinstall, generate the set on demand:

```bash
npx iconify-json-nucleo-codegen build --base node_modules/iconify-json-nucleo-pixel
```
...
```ts
// ESM
import { icons } from 'iconify-json-nucleo-pixel';
...
const icons = require('iconify-json-nucleo-pixel');
...
```tsx
<Icon icon="nucleo-pixel:heart-24" />
```

With [`@iconify/tailwind`](https://iconify.design/docs/usage/css/tailwind/) the JSON is read straight from `node_modules`.

```html
<span class="icon-[nucleo-pixel--heart-24]"></span>
```

Build tools that need the raw set can read it directly from the `iconify-json-nucleo-pixel/icons.json` subpath export.
