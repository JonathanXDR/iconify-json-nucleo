...
| --- | --- |
| npm 12+ | Run `npm approve-scripts {{packageName}}` |
| pnpm 11+ | Add `{{packageName}}: true` under `allowBuilds` in `pnpm-workspace.yaml`, or run `pnpm approve-builds` |
| Yarn 4.14+ | Add `dependenciesMeta.{{packageName}}.built: true` to `package.json` |
| Bun | Add `{{packageName}}` to `trustedDependencies` in `package.json`, or run `bun pm trust {{packageName}}` |
